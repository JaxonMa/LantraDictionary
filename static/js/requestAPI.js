        ...
        if (data.related_char && data.related_char.length > 0) {
            ...
            data.related_char.forEach(char => {
                ...
                wordTag.addEventListener('click', async function() {
                    ...
                    try {
                        // 向后端API发送POST请求
                        const response = await fetch('/api/lookup/' + char, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                            body: JSON.stringify({ q: char })
                        });
                        ...
                        if (data.char) {
                            ...
                            // 返回错误信息
                            displayNoResult(char, data.error);
                        } else {
                            // 未知数据格式
                            displayNoResult(char, '返回数据格式不正确');
                        }
                    } catch (error) {
                        console.error('搜索请求失败:', error);
                        displayNoResult(char, `请求失败: ${error.message}`);
                    }
